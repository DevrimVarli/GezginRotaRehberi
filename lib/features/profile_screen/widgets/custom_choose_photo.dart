...
class _CustomChoosePhotoState extends ConsumerState<CustomChoosePhoto> {
  ImagePicker picker =
      ImagePicker(); // Resim seçmek için kullanılan ImagePicker nesnesi
  User user =
      FirebaseAuth
          .instance
          .currentUser!; // Firebase'den şu anki kullanıcıyı alıyoruz

  @override
  Widget build(BuildContext context) {
    TextTheme textTheme =
        Theme.of(context).textTheme; // Temadaki metin stillerini alıyoruz
    return ElevatedButton(
      onPressed: () {
        // Butona tıklanıldığında, profil fotoğrafı seçmek ve yüklemek için pickAndUploadProfilePicture fonksiyonu çağrılır
        ref
            .read(authProvider)
            .pickAndUploadProfilePicture(picker: picker, user: user);
      },
